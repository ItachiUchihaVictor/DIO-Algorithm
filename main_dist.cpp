...
//    }
   ...
   std::cout << "reading query information finished." << std::endl;
    double begin, end;
   ...
   double GBtotalint, MMPstotalint, MMPedges, MMPstotaledges;
   double distance;

//   std::ofstream output(std::string(argv[1])+"_Sp.txt", std::ios::out );
   std::ofstream GB(std::string(argv[1])+"_dist_gb.txt", std::ios::out );
...
//   std::ofstream MMP(std::string(argv[1])+"_mmp.txt", std::ios::out | std::ios::app);
   std::ofstream MMPs(std::string(argv[1])+"_dist_mmps.txt", std::ios::out );
    ...
    for(int i=0;i<10;i++){
      ...
      for(int j=0;j<100;j++){
        int src = srcs[i][j]; // rand()*rand()%mesh.vertices().size();
        int dst = dsts[i][j]; // rand()*rand()%mesh.vertices().size();
...
//        }
        ...
        std::cout << distance << " " << GBtime << " " << GBinterval << " " << GBedges/2.0 << std::endl;
/*        begin = clock();
...
*/
        ...
        MMPstotaledges+=MMPedges/2.0;
        std::cout << distance << " " << MMPstime << " " << MMPsinterval << " " << MMPedges/2.0 << std::endl;
    }
        GB << i << " " << GBt/100.0 << " " << GBtotalint/100.0 << " " << GBtotaledges/200.0 << std::endl;
        MMPs << i << " " << MMPst/100.0 << " " << MMPstotalint/100.0 << " " << MMPstotaledges/200.0 << std::endl;
    }
